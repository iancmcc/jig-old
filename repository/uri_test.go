...
func assertURIParts(uri RepositoryURI, c *C) {
...
func (s *UriSuite) TestParseSSHUri(c *C) {
	...
	c.Assert(string(uri.Scheme()), Equals, SCHEME_SSH)
	gituri := uri.(*GitURI)
...
func (s *UriSuite) TestParseNoRepoUri(c *C) {
	...
	c.Assert(err, IsNil)
	assertURIParts(uri, c)
	c.Assert(uri.Scheme(), Equals, SCHEME_HTTPS)
}
...
func (s *UriSuite) TestParseGitUri(c *C) {
	...
	c.Assert(err, IsNil)
	assertURIParts(uri, c)
	c.Assert(string(uri.Scheme()), Equals, SCHEME_GIT)
}
